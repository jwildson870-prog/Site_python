...
# Configurações administrativas persistentes do Portal Python.
...
def get_system_setting(key, default=None):
    ...
    if row is not None:
        return row.value
    return SYSTEM_SETTING_DEFAULTS.get(key, default)

def get_system_bool(key, default=False):
    raw = get_system_setting(key, 'true' if default else 'false')
    return str(raw).strip().lower() in {'1', 'true', 'yes', 'on', 'sim'}

def get_system_int(key, default=0):
    try:
        return int(get_system_setting(key, str(default)))
